...
object Entry_ChooseBasisForClaimPage extends BasePage {
  ...
  override def checkContent(content: String): Unit = {
    ...
    driver.findElement(By.cssSelector("#main-content > div > div > details > div > ul > li:nth-child(13)")).getText should equal("Incorrect excise value - Northern Ireland claims only: Correction to excise value because you’ve made a mistake when calculating excise due for duty-paid goods (only applies to movements into Northern Ireland from Great Britain). Note: under this basis for claim, if you imported into Northern Ireland, you can only claim under excise tax codes and not under customs tax codes or import VAT.")
