...
object EnterMovementReferenceNumberPage extends BasePage {
  ...
  override def checkPageTitle(content:String): Assertion = {
    content match {
      case "entry-disabled" => driver.getTitle should equal("What is your Movement Reference Number (MRN) - Claim for reimbursement of import duties - GOV.UK")
      case "entry-enabled"  | _ => driver.getTitle should equal("Enter the Movement Reference Number (MRN) - Claim for reimbursement of import duties - GOV.UK")
    }
  }
  ...
  override def checkPageHeader(content:String): Assertion = {
    content match {
      case "entry-disabled" => driver.findElement(By.cssSelector("h1")).getText should equal("What is your Movement Reference Number (MRN)")
      case "entry-enabled" | _  => driver.findElement(By.cssSelector("h1")).getText should equal("Enter the Movement Reference Number (MRN)")
    }
  }
