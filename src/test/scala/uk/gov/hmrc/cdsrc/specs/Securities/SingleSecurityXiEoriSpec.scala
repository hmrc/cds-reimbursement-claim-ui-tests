...
class SingleSecurityXiEoriSpec extends BaseSpec {
  ...
  Feature("Securities Single - XIEORI") {
    ...
    Scenario("happy path - check importer/declarant eori pages - with no Payment method") {
      ...
      When("User enters 01XIDCAAAAAAAAAAA2 on Securities Enter Movement Reference Number Page")
      Securities_EnterMovementReferenceNumberPage.waitForPageHeader
      Securities_EnterMovementReferenceNumberPage.enterDetails("01XIDCAAAAAAAAAAA2")
      ...
      Then("User is presented with the Securities Enter Importer Eori Page")
      Securities_EnterImporterEoriPage.waitForPageHeader
      Securities_EnterImporterEoriPage.checkURL
      Securities_EnterImporterEoriPage.checkPageHeader()
      Securities_EnterImporterEoriPage.checkPageTitle()

      When("User enters XI000000000000002 on Securities Enter Importer Eori Page")
      Securities_EnterImporterEoriPage.waitForPageHeader
      Securities_EnterImporterEoriPage.enterDetails("XI000000000000002")

      And("User clicks continue on Securities Enter Importer Eori Page")
      Securities_EnterImporterEoriPage.waitForPageHeader
      Securities_EnterImporterEoriPage.clickContinueButton()

      Then("User is presented with the Securities Enter Declarant Eori Page")
      Securities_EnterDeclarantEoriPage.waitForPageHeader
      Securities_EnterDeclarantEoriPage.checkURL
      Securities_EnterDeclarantEoriPage.checkPageHeader()
      Securities_EnterDeclarantEoriPage.checkPageTitle()

      When("User enters XI000000000000002 on Securities Enter Declarant Eori Page")
      Securities_EnterDeclarantEoriPage.waitForPageHeader
      Securities_EnterDeclarantEoriPage.enterDetails("XI000000000000002")

      And("User clicks continue on Securities Enter Declarant Eori Page")
      Securities_EnterDeclarantEoriPage.waitForPageHeader
      Securities_EnterDeclarantEoriPage.clickContinueButton()
      ...
      Then("User is presented with the Securities MulCheck Declaration Details Page")
      ...
      And("User clicks continue on Securities MulCheck Declaration Details Page")
      Securities_MulCheckDeclarationDetailsPage.waitForPageHeader
      ...
      Then("User is presented with the Securities Confirm Full Repayment Security Id Page")
      Securities_ConfirmFullRepaymentSecurityIdPage.waitForPageHeader
      Securities_ConfirmFullRepaymentSecurityIdPage.checkURL
      Securities_ConfirmFullRepaymentSecurityIdPage.checkPageHeader()
      Securities_ConfirmFullRepaymentSecurityIdPage.checkPageTitle()

      When("User selects radio button Yes on Securities Confirm Full Repayment Security Id Page")
      Securities_ConfirmFullRepaymentSecurityIdPage.waitForPageHeader
      Securities_ConfirmFullRepaymentSecurityIdPage.clickRadioButton("Yes")

      And("User clicks continue on Securities Confirm Full Repayment Security Id Page")
      Securities_ConfirmFullRepaymentSecurityIdPage.waitForPageHeader
      Securities_ConfirmFullRepaymentSecurityIdPage.clickContinueButton()

      Then("User is presented with the Securities MulCheck Claim Page")
      ...
      And("User clicks continue on Securities MulCheck Claim Page")
      Securities_MulCheckClaimPage.waitForPageHeader
      ...
      val expectedData = Map(
        "Import Movement Reference Number (MRN)"         -> "01XIDCAAAAAAAAAAA2",
        "Reason for security deposit"                    -> "Temporary Admission (24 months)",
        "Local Reference Number (LRN)"                   -> "XFGLKJDSE5GDPOIJEW985T",
        "Uploaded"                                       -> "document.pdf,Import declaration",
        "Total security deposit value"                   -> "£14,085.52",
        "What did you do with the goods?"                -> "Declared to a customs warehouse",
        "Contact address"                                -> "12 Skybricks Road,Coventry,CV3 6EA,United Kingdom",
        "Claim for security deposit or guarantee 1 of 5" -> "Yes",
        "Claim for security deposit or guarantee 2 of 5" -> "No",
        "Claim for security deposit or guarantee 3 of 5" -> "No",
        "Claim for security deposit or guarantee 4 of 5" -> "No",
        "Duties selected"                                -> "A00 - Customs Duty,B00 - Value Added Tax (VAT)",
        "B00 - Value Added Tax (VAT)"                    -> "£8,085.52",
        "A00 - Customs Duty"                             -> "£6,000.00",
        "Total"                                          -> "£14,085.52",
        "Claim for security deposit or guarantee 5 of 5" -> "No",
        "Do you want to provide more detail?"            -> "under 500 characters",
        "Claim full amount"                              -> "Yes",
        "Payee"                                          -> "Importer",
        "Sort code"                                      -> "123456",
        "Date security deposit made"                     -> "13 September 2019",
        "Contact details"                                -> "Mr John Smith,someemail@mail.com,01234567890",
        "Name on the account"                            -> "Mr John Smith",
        "Account number"                                 -> "11001001",
