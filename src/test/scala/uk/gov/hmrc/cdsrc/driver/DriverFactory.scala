...
import uk.gov.hmrc.cdsrc.driver.DriverFactory.BrowserExtensions
...
class DriverFactory extends LazyLogging {
  ...
  private val edgeBrowserVersion    = TestConfiguration.browserVersion
  private val firefoxBrowserVersion = TestConfiguration.browserVersion
  private val chromeBrowserVersion  = TestConfiguration.browserVersion
  ...
  def firefoxOptions(): FirefoxOptions = {
  ...
  def browserLogging(capabilities: MutableCapabilities): MutableCapabilities = {
    val browserName = capabilities.getBrowserName

    if (TestConfiguration.browserLoggingEnabled)
      browserName match {
        case "chrome" =>
  ...
  def accessibilityAssessment(capabilities: MutableCapabilities): MutableCapabilities = {
    val browserName = capabilities.getBrowserName

    if (TestConfiguration.accessibilityAssessmentEnabled)
      browserName match {
        case "chrome"        =>
          capabilities
  ...
  def securityAssessment(capabilities: MutableCapabilities): MutableCapabilities = {
    ...
    if (TestConfiguration.securityAssessmentEnabled) {
      proxy.setHttpProxy(TestConfiguration.zapHost)
      proxy.setSslProxy(TestConfiguration.zapHost)
      ...
      logger.info(s"Security assessment: Enabled (${TestConfiguration.zapHost})")
    }

    capabilities
  }

  def headless(capabilities: MutableCapabilities): MutableCapabilities = {
    val browserName = capabilities.getBrowserName

    if (TestConfiguration.browserOptionHeadLessEnabled) {
      browserName match {
        case "chrome"        =>
          capabilities
            .asInstanceOf[ChromeOptions]
